...
<nav class="bg-transparent p-5 flex justify-between items-center">
    ...
    <div class="container mx-auto py-8">
        ...
        <form method="get" id="filterForm" class="mb-4">
            ...
            <a href="FilmUpload.php"><button type="submit" class="ml-2 mt-2 mb-4 bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">
                Upload New Film
            </button></a>
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            <div class="bg-white rounded-lg shadow-md overflow-hidden">
                <img src="<?php echo htmlspecialchars($row["PosterImage"]); ?>" alt="<?php echo htmlspecialchars($row["Titel"]); ?>" class="w-full">
                <div class="p-4">
                    <h2 class="text-lg font-semibold mt-2 mb-1"><?php echo "<a href='filmdetails.php?filmId=" . $row["FilmID"] . "'>" . htmlspecialchars($row["Titel"]) . "</a> "?></h2>
                    <p class="text-gray-500 mb-2">Genres: <?php echo htmlspecialchars($row["Genres"]); ?></p>
                    <p class="text-gray-500 mb-2">Regisseur: <?php echo htmlspecialchars($row["Regisseurnaam"]); ?></p>
                    <p class="text-gray-500 mb-2">Releasejaar: <?php echo htmlspecialchars($row["Releasejaar"]); ?></p>
                    <p class="text-gray-500 mb-2"><?php echo htmlspecialchars($row["Beschrijving"]); ?></p>
                    <div class="flex justify-between mt-4">
                        <a href="phpexecutables/update.php?filmId=<?php echo $row["FilmID"]; ?>" class="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded">
                            Update
                        </a>
                        <form action="phpexecutables/delete.php" method="post" onsubmit="return confirm('Are you sure you want to delete this film?')">
